...
| 구분 | 내용 |
|---|---|
| **특징** | 통증 심함 · 발열 · 개구장애(입 벌리기 힘듦) · 목소리 변화(감자 먹은 목소리) |
| **치료** | 절개배농(I&D) 또는 바늘흡인으로 물리적 고름 제거 |
| **간병 포인트** | 절개 부위 청결 유지 · 통증·발열 체크 · 항생제 투여 시간 준수 · 수분/영양 섭취 격려(삼키기 힘들어함) |

#### ■ 호전기 (회복기)

| 구분 | 내용 |
|---|---|
| **특징** | 농양 크기 감소 · 통증 완화 · 개구장애 호전 |
| **체내 기전** | 항생제로 세균 억제 + 면역세포(대식세포)가 남은 염증 산물을 분해·흡수 → 림프/정맥으로 서서히 처리 |
| **⚠️ 핵심 주의** | 이 시기에 **마사지나 압박으로 고름을 짜내려 하면 안 됨** — 감염이 주변 조직(인접 근막 공간, 예: 인두주위 공간 등)으로 퍼질 위험 |
| **온찜질** | 통증 완화·혈류 개선 목적으로는 가능하나, **"배농 촉진용 압박"은 금지** |
| **간병 포인트** | 절개 부위 자연 치유 관찰 · 식사량 서서히 늘리기(부드러운 음식부터) · 통증 재발/발열 재상승 시 즉시 보고 |

### 6-2. ⚠️ 본 환자 적용 시 반드시 확인할 사항 (마사지 관련)

현재 케어 중인 "외부 마사지"는 환자가 지금 **호전기**(PCT 정상화 12일차, CRP 1.7~1.9 잔여)라는 점에서 아래 원칙이 특히 중요합니다:

1. **목 앞쪽·붓은 부위를 "짜내듯" 누르거나 밀어내는 마사지는 금지** — 남은 고름/염증물이 인접 근막 공간(인두주위 공간 등)으로 밀려 퍼질 수 있음
2. 가능한 마사지: **목 뒤·어깨·턱 바깥 근육 이완용 가벼운 쓸어내리기(stroking)** 수준 — 절개 부위·붓기 부위 직접 압박 없이
3. 디클로메드(국소 소염제)는 마사지하듯 문지르는 용도가 아니라 **패치/겔 부착** 목적 — 부착 부위를 압박하지 말 것
4. 온찜질은 통증 완화 목적으로 OK — 단, 확실히 붓고 열감 있는 부위에는 의료진 확인 후 적용
5. 가글(탐튬)·구강 세척(크린조)은 **입안 쪽 배농 경로를 관리하는 정상 처치**이므로 계속 유지 — "외부에서 짜낸다"는 개념과는 다름

> 요약: **"입안 처치(가글·세척) = 계속", "외부 압박·짜내기 = 금지"** — 이 구분만 확실히 지키면 됩니다.

### 6-3. 공통 체크리스트 (매일 확인)

- [ ] 항생제 스케줄 엄수 (투여 시간 정확히)
- [ ] 발열 체크 — 재상승(38℃↑) 시 즉시 의료진 알림
- [ ] 통증 악화 여부 — 진통제 간격이 좁아지면 보고
- [ ] 삼킴곤란 재발 여부
- [ ] 절개 부위 관찰: 붓기 · 발적 · 분비물 양/색/냄새 변화
- [ ] 입 벌리기 정도(개구장애) 호전 추이
- [ ] 식사량·수분 섭취량 기록
- [ ] 혈당 수치 인지 (TPN·인슐린 중인 당뇨 환자)

### 6-4. 간병 내용 × 검사 수치 연결표

| 간병 행동 | 대응 수치 | 함께 보면 좋은 신호 |
|---|---|---|
| 구강 처치·가글 성실 수행 | CRP 1.7~1.9 → 1.0 미만 목표 | 분비물 감소, 입냄새 개선 |
| 식사량 서서히 늘리기 | 알부민 3.5 → 4.0 목표 | TPN 용량 감소, 혈당 변동 폭 축소 |
| 항생제 시간 엄수 | PCT <0.05 유지 | 발열 없음 |
| 무리한 마사지·압박 금지 | — (재악화 예방) | 붓기·통증 재발 여부 |
| 통증·발열 즉시 보고 | WBC·CRP 재상승 조기 발견 | 4-1의 재악화 경보 기준 참조 |
